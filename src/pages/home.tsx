...
function StatItem({ value, label, prefix = "", suffix = "" }: { value: number; label: string; prefix?: string; suffix?: string }) {
  ...
  return (
    <div className="text-center">
      ...
      <div className="mt-1 text-sm text-navy-300">{label}</div>
    </div>
  );
}
    ...
    <div>
        ...
        <div className="mx-auto max-w-6xl px-5 sm:px-6 py-16 sm:py-24">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              {/* Badge */}
              <div
                className="mb-5 inline-flex items-center gap-2 rounded-full border border-coral-200 bg-coral-50 px-4 py-1.5 text-sm font-semibold text-coral-600"
              ...
              {/* Heading */}
              <h1
                className="text-4xl font-black leading-tight text-navy-900 sm:text-5xl lg:text-6xl"
            ...
            >
              <div className="relative">
                <div className="bg-coral-50 w-64 h-80 rounded-2xl flex items-center justify-center shadow-lg">
                  <div className="text-center">
                    ...
                    <p className="mt-3 text-sm font-bold text-coral-600">BooksWithYou</p>
                  </div>
                </div>
                {/* Floating mini cards */}
                <div className="absolute -top-4 -right-4 bg-teal-100 border border-teal-200 rounded-xl p-3 shadow-md" style={{ animation: "auroraDrift 8s ease-in-out infinite" }}>
                  <svg className="h-6 w-6 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <div className="absolute -bottom-3 -left-3 bg-amber-50 border border-amber-200 rounded-xl p-3 shadow-md" style={{ animation: "auroraDrift 10s ease-in-out infinite reverse" }}>
      ...
      <section className="reveal py-16 sm:py-20" ref={revealRefs[0]}>
        <div className="mx-auto max-w-5xl px-5 sm:px-6">
          ...
          <h2 className="mt-2 text-center text-3xl font-extrabold sm:text-4xl text-navy-900">
          ...
          <div className="mt-12 grid sm:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="text-center">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-teal-100 text-teal-600 text-xl font-extrabold mb-4">
                  {item.step}
                </div>
                <div className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-coral-50 text-coral-500 mb-3">
      ...
      {/* ──────────────────────── Features ──────────────────────── */}
      <section className="reveal py-16 sm:py-20 bg-white" ref={revealRefs[1]}>
        <div className="mx-auto max-w-5xl px-5 sm:px-6">
          ...
          <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: "Public & Private", desc: "Open sessions for all to join, or keep them invite-only.", bg: "bg-white", iconColor: "text-coral-500", iconBg: "bg-coral-50" },
              { title: "Chapter Tracking", desc: "Break books into chapters and track progress precisely.", bg: "bg-teal-50", iconColor: "text-teal-600", iconBg: "bg-teal-100" },
              { title: "Live Discussions", desc: "Chat in real time with fellow readers in the session.", bg: "bg-amber-50", iconColor: "text-amber-600", iconBg: "bg-amber-100" },
              { title: "Emoji Reactions", desc: "React to comments with emojis. Simple and expressive.", bg: "bg-coral-50", iconColor: "text-coral-500", iconBg: "bg-coral-100" },
            ].map((f, i) => (
              <div key={i} className={`${f.bg} rounded-2xl border border-slate-100 p-6 hover:shadow-md transition-shadow`}>
      ...
      <section className="reveal py-16 sm:py-20" ref={revealRefs[3]}>
        <div className="mx-auto max-w-5xl px-5 sm:px-6">
          ...
          <div className="flex items-end justify-between">
            <h2 className="mt-2 text-3xl font-extrabold sm:text-4xl text-navy-900">
      ...
      {/* ──────────────────────── Progress Demo ──────────────────────── */}
      <section className="reveal py-16 sm:py-20 bg-white" ref={revealRefs[4]}>
        <div className="mx-auto max-w-5xl px-5 sm:px-6">
          <span className="section-label">Track Together</span>
          <h2 className="mt-2 text-center text-3xl font-extrabold sm:text-4xl text-navy-900">
            See everyone&apos;s progress in real time
          </h2>
          <p className="mt-3 text-center text-slate-500 max-w-xl mx-auto mb-10">
          ...
          <div className="max-w-2xl mx-auto bg-white border border-slate-200 rounded-2xl shadow-sm p-8">
            <div className="mb-6 pb-5 border-b border-slate-100">
              <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Currently reading</p>
              <h3 className="text-xl font-bold text-navy-900">Project Hail Mary</h3>
              <p className="text-sm text-slate-500">by Andy Weir</p>
            </div>
              ...
              {DEMO_MEMBERS.map((m, i) => (
                <div key={i}>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="ml-auto text-xs text-slate-400">{m.chapters} / {m.total} chapters</span>
                  </div>
        ...
        <div className="mx-auto max-w-5xl px-5 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {TESTIMONIALS.map((t, i) => (
              <blockquote key={i} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                ...
                <footer className="mt-4 flex items-center gap-3">
                  ...
                  <div>
                    <cite className="not-italic text-sm font-semibold text-navy-900">{t.name}</cite>
